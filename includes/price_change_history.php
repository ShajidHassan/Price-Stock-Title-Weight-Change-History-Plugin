...
class Price_Change_History_List_Table extends WP_List_Table
{
    ...
    public function prepare_items()
    {
        ...
        $this->items = $wpdb->get_results(
            "SELECT product_id, price, change_date, changed_by
        FROM $table_name
        $where_clause_without_limit
        ORDER BY change_date DESC"
        );

        // Calculate and store the price difference for each item
        foreach ($this->items as &$item) {
            $product = wc_get_product($item->product_id);
            if ($product) {
                $item->price_difference = $product->get_price() - $item->price;
            } else {
                $item->price_difference = 0; // Default to 0 if product not found
            }
        }
        $filtered_items = [];
        ...
        switch ($selected_price_difference) {
            case 'lt0':
                $filtered_items = array_filter($this->items, function ($item) {
        ...
        // Update pagination args based on the filtered items for price difference
        $total_filtered_items = count($filtered_items);
        $current_page = $this->get_pagenum();
        $per_page = $this->get_items_per_page('price_change_history_per_page', 50);

        // Set the pagination arguments only if there are filtered items
        if ($total_filtered_items > 0) {
            ...
            $this->items = array_slice($filtered_items, ($current_page - 1) * $per_page, $per_page);
        } else {
            // If there are no filtered items, set total items to 0 to prevent pagination display
            $this->set_pagination_args(array(
                'total_items' => 0,
                'per_page'    => $per_page,
            ));
            $this->items = []; // Set items to empty to prevent displaying any data
        }
    }
    ...
    public function get_columns()
    {
        $columns = array(
            ...
            'new_price'    => __('Current Price', 'price-change-history'),
    ...
    public function column_default($item, $column_name)
    {
        switch ($column_name) {
            ...
            case 'new_price':
                ...
                if ($product && $product->get_price() > $item->price) {
                    return wc_price($product->get_price()) . ' <span class="dashicons dashicons-arrow-up-alt" style="color:green;"></span>' . ' (' . wc_price($product->get_price() - $item->price) . ')';
                } elseif ($product && $product->get_price() < $item->price) {
                    return wc_price($product->get_price()) . ' <span class="dashicons dashicons-arrow-down-alt" style="color:red
                    ;"></span>' . ' (' . wc_price($product->get_price() - $item->price) . ')';
                }
